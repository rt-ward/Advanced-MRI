function [imaging_frequency,delta_TE,TE,B0_dir,B0_mag,pdf_tol,pdf_n_cg,pdf_space,pdf_n_pad,csf_flag_erode,csf_thresh_R2s,medi_tol_norm_ratio,medi_max_iter,medi_cg_verbose,medi_cg_max_iter,medi_cg_tol,medi_lambda,medi_msmv,load_nifti_common_prefix,load_negate_every_other_axis,phase_corr,invert_phase,phase_encoding_dir,method_phase_unwrap,prefilter,bipolar_complex_fit,debug_mode] = load_pipeline_parameters(path_parameters_json)
    % Default pipeline parameters:
    imaging_frequency = 127.7;
    ...
    bipolar_complex_fit = 0; % 1 for fit_ppm_complex_bipolar, 0 for fit_ppm_complex
    debug_mode = 0; % 1 for generating additional temporary files and keeping all temporary files (increases memory usage), 0 for deleting temporary files
    ...
    if isfile(path_parameters_json)
        ...
        if isfield(json_data,'ImagingFrequency')  %CF aka ImagingFrequency * 1e6
            ...
            disp("INFO: Loaded ImagingFrequency from json as " + imaging_frequency + " MHz")
        end
        ...
        if isfield(json_data,'phase_corr')
            phase_corr = import_as_integer(json_data.phase_corr);
        ...
        if isfield(json_data,'invert_phase')
            invert_phase = import_as_integer(json_data.invert_phase);
        ...
        if isfield(json_data,'prefilter')
            prefilter = import_as_integer(json_data.prefilter);
        ...
        if isfield(json_data,'bipolar_complex_fit')
            bipolar_complex_fit = import_as_integer(json_data.bipolar_complex_fit);
            if bipolar_complex_fit == 1 || bipolar_complex_fit == 0
                disp("INFO: Loaded bipolar_complex_fit from json as " + bipolar_complex_fit)
            else
                num_errors_encountered = num_errors_encountered + 1;
                disp("ERROR: bipolar_complex_fit in json file must have a value of either 1 or 0, (true or false, respectively)");
            end
        end
        if isfield(json_data,'debug_mode')
            debug_mode = import_as_integer(json_data.debug_mode);
            if debug_mode == 1 || debug_mode == 0
                disp("INFO: Loaded debug_mode from json as " + debug_mode)
            else
                num_errors_encountered = num_errors_encountered + 1;
                disp("ERROR: debug_mode in json file must have a value of either 1 or 0, (true or false, respectively)");
            end
        end
        ...
        if isfield(json_data,'csf_flag_erode')
            csf_flag_erode = import_as_integer(json_data.csf_flag_erode);
            if invert_phase == 1 || invert_phase == 0
...
function parsed_value = import_as_integer(value)
    if isstring(value) || ischar(value) || iscellstr(value)
        temp_val = str2double(value);
        parsed_value = uint8(temp_val);
    else
        parsed_value = value;
    end
end
