function [iField,magnitude,phase,voxel_size,matrix_size,CF,delta_TE,TE,B0_dir,B0_mag,phase_encoding_dir]=load_nifti_folder(path_work,load_negate_every_other_axis,default_imaging_frequency)
    ...
    if is_single_file == 1
        ...
        if isfield(data,'ImagingFrequency') == 0
            disp("WARN: load_nifti_folder.m - ImagingFrequency not found in json file, setting ImagingFrequency as " + default_imaging_frequency + " MHz")
            data.ImagingFrequency = default_imaging_frequency;
        end
        if isfield(data,'InPlanePhaseEncodingDirectionDICOM') == 0
            disp("WARN: load_nifti_folder.m - InPlanePhaseEncodingDirectionDICOM not found in json file, setting InPlanePhaseEncodingDirectionDICOM as 'NOT SET'")
            data.InPlanePhaseEncodingDirectionDICOM = default_phase_encoding_dir;
        else
        ...
        for echo = 1:numEchoes
            if combine_real_imag == 1
                disp("INFO: load_nifti_folder.m - using real and imaginary from multiple echo files - echo " + echo)
...
%                disp(max(max(max(echoImag))))
                ...
                disp("INFO: load_nifti_folder.m - using magnitude and phase from multiple echo files - echo " + echo')
            ...
            if echo == 1
                ...
                if isfield(data,'InPlanePhaseEncodingDirectionDICOM') == 0
                    disp("WARN: load_nifti_folder.m - InPlanePhaseEncodingDirectionDICOM not found in json file")
                else
                    phase_encoding_dir = data.InPlanePhaseEncodingDirectionDICOM;
                end
                CF = data.ImagingFrequency*1e6;
    ...
    if combine_real_imag == 1
        % when combining real and mag to complex, phase needs to be negated
        phase = phase*-1;
    end
